...
Route::group(['prefix' => 'teacher'], function() {
    Route::group(['middleware' => 'teacher.guest'], function(){
        ...
        Route::get('register',[\App\Http\Controllers\Teacher\Auth\TeacherRegisterController::class,'register'])->name('teacher.register');
        Route::any('register/store',[\App\Http\Controllers\Teacher\TeacherController::class,'store'])->name('teacher.register.store');

        Route::view('/login','teacher.login')->name('teacher.login');
        Route::post('/login',[\App\Http\Controllers\Teacher\TeacherController::class, 'authenticate'])->name('teacher.auth');
    });

    Route::group(['middleware' => 'teacher.auth'], function(){
        Route::get('/dashboard',[\App\Http\Controllers\Teacher\TeacherController::class, 'dashboard'])->name('teacher.dashboard');

        Route::get('logout',[\App\Http\Controllers\Teacher\TeacherController::class,'logout'])->name('teacher.logout');
        ...
        Route::get('/department_view',[\App\Http\Controllers\DepartmentController::class,'view'])->name('view.department');
        ...
        Route::get('/answer', [\App\Http\Controllers\Teacher\TeacherController::class, 'teacher_answer'])->name('teacher.answers');
    });
});
